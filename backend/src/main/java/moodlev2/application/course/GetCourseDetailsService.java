...
public class GetCourseDetailsService {
    ...
    public CourseDetailsResponse getCourseDetails(String idOrCode, String userEmail) {
        ...
        if (idOrCode.matches("\\d+")) {
            ...
            course = courseRepository.findByCode(idOrCode.toUpperCase())
                    .orElseThrow(() -> new NotFoundException("Course not found with code: " + idOrCode));
        }
        ...
        List<CourseModuleItemDto> allQuizzes = course.getQuizzes().stream()
        ...
        CourseModuleDto activeModule = modules.stream()
                .filter(m -> "current".equals(m.status()))
                .findFirst()
                .orElse(null);
        ...
        int completedModules = (int) modules.stream().filter(m -> "completed".equals(m.status())).count();
        ...
        CourseCurrentModuleDto currentModule = new CourseCurrentModuleDto(currentTitle, overallProgress, dueLabel);
    ...
    private CourseModuleDto mapModuleWithStatus(CourseModuleEntity module, UserEntity user) {
        ...
        Stream<CourseModuleItemDto> fileItems = module.getItems().stream()
        ...
        Stream<CourseModuleItemDto> quizItems = module.getQuizzes().stream()
                .filter(q -> "PUBLISHED".equalsIgnoreCase(q.getStatus()))
    ...
    private CourseModuleItemDto mapQuizToItem(QuizEntity quiz, UserEntity user) {
        ...
        if (quiz.getMaxAttempts() != null && quiz.getMaxAttempts() > 0) {
            int attemptsUsed = quizAttemptRepository.countByQuizIdAndUserId(quiz.getId(), user.getId());
